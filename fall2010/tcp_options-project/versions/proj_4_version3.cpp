...
//printf( "%s\n\n", inet_ntoa(ip->ip_src));
	//printf("%s\n\n", inet_ntoa(ip->ip_dst));
	if(ntohs(tcp->th_sport) >1000 && ntohs(tcp->th_dport) <1000)//******************* Client to s*************************
	{
		ctos_total++;
		if(  (tcp->th_flags & (TH_SYN | TH_FIN | TH_ACK))!=0 )
		{
			c2s_syn_fin_ack++;
		}
		else
		{
			if( (tcp->th_flags & (TH_SYN | TH_ACK))!=0  )
			{
				c2s_syn_ack++;
			}
			else if( (tcp->th_flags & (TH_FIN | TH_ACK))!=0 )
			{
				c2s_fin_ack++;
			}
			else
...
//*******************Sequence Number***********************//
	...
	if(ntohs(tcp->th_sport) <1000 && ntohs(tcp->th_dport) > 1000) //****   Server to C  *********************************
	{
		stoc_total++;
if(  (tcp->th_flags & (TH_SYN | TH_FIN | TH_ACK))!=0 )
		{
