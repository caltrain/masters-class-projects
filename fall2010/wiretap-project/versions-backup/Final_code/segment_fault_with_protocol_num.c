...
int main(int argc, char **argv)
{
	...
	{  /*******************Independent block*****************/
			int len;
			char iptemp[30], ip_check[30];
			int k, j,ipCount=0, srcmax=0, desmax=0 ,ipMaxCount=0;
			...
			for(k=0;k<srcmax;k++)    //outer loop
			{
				...
				{
					if(strcmp(ip_src[k].ip,flow_sum[k].ip_src) != 0 && strcmp(ip_des[k].ip, flow_sum[k].ip_des)!=0 )
					{

							strcpy(flow_sum[k].ip_src,ip_src[k].ip);
							strcpy(flow_sum[k].ip_des,ip_des[k].ip);

							flow_sum[k].one_two = 1;
		...
		printf("IP 1 \t\t  IP 2 \t\t   #Packet 1-2  \t\t  #Packet 2-1");
		for(k=0;k< ipMaxCount ; k++)
		{
				printf("   %s\t\t %s \t\t  %d\n",RightPad( flow_sum[k].ip_src, 20," "), RightPad( flow_sum[k].ip_des, 20," "), flow_sum[k].one_two);
		}
